(function(module) {
  ...
  module.controller('EditCustomerMissionController', ['$scope', '$rootScope', '$anchorScroll', '$location', '$stateParams', 'missionRequest', 'Mission', 'MissionAction', 'projectService', 'listCustomerService', 'missionCustomerService', 'participantList', 'mapData', 'alertsService', '$modal', '$state', '$log', 'missionPreviewService', 'modalService', '$filter', '$rootScope', 'taOptions',
    function($scope, $rootScope, $anchorScroll, $location, $stateParams, missionRequest, Mission, MissionAction, projectService, listCustomerService, missionCustomerService, participantList, mapData, alertsService, $modal, $state, $log, missionPreviewService, modalService, $filter, $rootScope, taOptions) {
          ...
          if (!$scope.equalsfunc() && !confirmedExit) {
            ...
            modalInstance.result.then(function(result) {
              if (result.save === true) {
                ...
                missionCustomerService.updateMission($scope.mission, false).then(function(response) {
                  confirmedExit = true;
                  $state.go(toState);
      ...
      $scope.addProjectTeam = function() {
        var modalInstance = modalService.addCustomerList();
        modalInstance.result.then(function(result) {
          listCustomerService.searchlistCustomer({
            count: 0,
            page: 0
        ...
        if (getDaysDifference($scope.mission.startDate, new Date()) < 0) {
          ...
          modalInstance.result.then(function(result) {
            if (result.send === true) {
              ...
              //$state.go('index.secured.customermission.search');
            }
          });
        } else {
          isValid = true;
      ...
      function populateMissionScreen() {
            ...
            angular.forEach(mapData.touchpoints, function(touchpointData, touchpointKey) {
              if (touchpointData.actions.length > 0) {
                angular.forEach(touchpointData.actions, function(actionData, actionKey) {
                  ...
                  if (actionData.feedbackSummary.CUSTOMERS.chartSummary.expectation === null) {
                    actionData.feedbackSummary.CUSTOMERS.chartSummary.expectation = new Object();
                    actionData.feedbackSummary.CUSTOMERS.chartSummary.expectation.averageAnswer = 6;
                  }

                  //if frequency is not answered
                  if (actionData.feedbackSummary.CUSTOMERS.chartSummary.frequency === null) {
                    actionData.feedbackSummary.CUSTOMERS.chartSummary.frequency = new Object();
                    actionData.feedbackSummary.CUSTOMERS.chartSummary.frequency.averageAnswer = 6;
                  }

                  //if loyalty is not answered
                  if (actionData.feedbackSummary.CUSTOMERS.chartSummary.loyalty === null) {
                    actionData.feedbackSummary.CUSTOMERS.chartSummary.loyalty = new Object();
                    actionData.feedbackSummary.CUSTOMERS.chartSummary.loyalty.averageAnswer = 6;
                  }

                  if ($scope.projectData.curveType === "EMOTION") {
                    //if emotion is not answered
                    if (actionData.feedbackSummary.CUSTOMERS.chartSummary.emotion === null) {
                      actionData.feedbackSummary.CUSTOMERS.chartSummary.emotion = new Object();
                      actionData.feedbackSummary.CUSTOMERS.chartSummary.emotion.averageAnswer = 6;
                    }

                  } else {
                    //if value is not answered
                    if (actionData.feedbackSummary.CUSTOMERS.chartSummary.value === null) {
                      actionData.feedbackSummary.CUSTOMERS.chartSummary.value = new Object();
                      actionData.feedbackSummary.CUSTOMERS.chartSummary.value.averageAnswer = 6;
                    }
                  }

                  var stagepositionIndex = mapData.positionIndex + 1;
                  var touchpointpositionIndex = touchpointData.positionIndex + 1;
                  var actionpositionIndex = actionData.positionIndex + 1;
                  ...
                  $scope.stages.push({
                    ...
                    "action": actionData,
                    "checked": checked,
                    "position": stagepositionIndex + '.' + touchpointpositionIndex + '.' + actionpositionIndex
                  });
         ...
         * @name colorCode
         * @description
         * row shading is done by according to the average point
         * colors are define in the mission.less
         */
         ...
         * @name scoreColor
         * @description
         * row shading color is selected according to the given selected sorting type
         * average is send to colorCode function
         */
        $scope.scoreColor = function(stage, sortType) {
          var expectationAverage = stage.action.feedbackSummary.CUSTOMERS.chartSummary.expectation.averageAnswer;
          var loyaltyAverage = stage.action.feedbackSummary.CUSTOMERS.chartSummary.loyalty.averageAnswer;
          if (stage.action.feedbackSummary.CUSTOMERS.chartSummary.value !== null) {
            var valueAverage = stage.action.feedbackSummary.CUSTOMERS.chartSummary.value.averageAnswer;
          } else {
            var emotionAverage = stage.action.feedbackSummary.CUSTOMERS.chartSummary.emotion.averageAnswer;
          }
          var frequencyAverage = stage.action.feedbackSummary.CUSTOMERS.chartSummary.frequency.averageAnswer;

          switch (sortType) {
            // case false:
            //   return false;
            //   break;
            case 'action.feedbackSummary.CUSTOMERS.chartSummary.expectation.averageAnswer':
              return colorCode(expectationAverage);
              break;
            case 'action.feedbackSummary.CUSTOMERS.chartSummary.loyalty.averageAnswer':
              return colorCode(loyaltyAverage);
              break;
            case 'action.feedbackSummary.CUSTOMERS.chartSummary.frequency.averageAnswer':
              return colorCode(frequencyAverage);
              break;
            case 'action.feedbackSummary.CUSTOMERS.chartSummary.emotion.averageAnswer':
              return colorCode(emotionAverage);
              break;
            case 'action.feedbackSummary.CUSTOMERS.chartSummary.value.averageAnswer':
      ...
      function updateMission(send) {
        $rootScope.showLoader('Updating...');
        missionCustomerService.updateMission($scope.mission, send).then(function(response) {
          alertsService.addAlert({
            title: 'Success',
          ...
          missionCustomerService.findMission($stateParams.id).then(function(data) {
            $scope.mission = data;
      ...
      $scope.copyMission = function(mission) {
        ...
        modalInstance.result.then(function() {
          ...
          missionCustomerService.copyMission(mission.id).then(function(data) {
            alertsService.addAlert({
              title: 'Success',
