(function(module) {
  ...
  module.controller('SearchPTMissionController', ['$scope', 'missionPTService', 'alertsService', '$state', 'searchManager', 'modalService', '$rootScope',
    function($scope, missionPTService, alertsService, $state, searchManager, modalService, $rootScope) {
      ...
      $scope.deleteProjectTeamMission = function(internalmission) {
        ...
        modalInstance.result.then(function() {
        	missionPTService.deleteMission(internalmission.id).then(function(data) {
      ...
      $scope.editProjectTeamMission = function(internalmission) {
        ...
        if (angular.equals(internalmission.status, "Scheduled")) {
          ...
          modalInstance.result.then(function() {
        	  missionPTService.setMissionToDraft(internalmission.id).then(function(data) {
                  alertsService.addAlert({
                    title: 'Success',
      ...
      $scope.copyProjectTeamMission = function(internalmission) {
          ...
          modalInstance.result.then(function() {
        	$rootScope.showLoader('Copying...');
          	missionPTService.copyMission(internalmission.id).then(function(data) {
              alertsService.addAlert({
                title: 'Success',
