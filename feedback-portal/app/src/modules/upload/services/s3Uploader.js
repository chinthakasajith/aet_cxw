(function(module) {
  ...
  module.service('s3Uploader', ['$http', '$q', '$window', '$rootScope', 's3Endpoint', '$log', 's3config',
    function($http, $q, $window, $rootScope, s3Endpoint, $log, s3config) {
      ...
      this.post = function(bucket, folder, acl, file) {
        ...
        return s3Endpoint.getPostParams(bucket, {
          acl: acl
      ...
      this.deleteS3Object = function(uri) {
        ...
        return s3Endpoint.getDeleteParams(bucket, key).then(function(response) {
          ...
          xhr.setRequestHeader('Host', s3config()+".s3.amazonaws.com");
