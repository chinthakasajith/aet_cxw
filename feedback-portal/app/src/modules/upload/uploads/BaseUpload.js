    ...
    function($q, $log) {
      ...
      BaseUpload.prototype.validate = function() {
        ...
        if (angular.isDefined(this.s3Options.maxsize)) {
          filesize = ((this.selectedFile.size / 1024) / 1024).toFixed(1);
          if (Number(filesize) > Number(this.s3Options.maxsize)) {
            isValidSize = false;
          }
        }
        //Horrible code. Fix this.
        ...
        if (!isValidType) {
          ...
          valid = false;
          self.$errors.push("Selected file size is " + filesize + "mb. File size must be less than " + this.s3Options.maxsize + "mb");
        }
